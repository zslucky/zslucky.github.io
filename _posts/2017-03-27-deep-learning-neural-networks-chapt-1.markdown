...
通常在网络中定义输入和输出是简单的。比如我们要确定一个手写数字图片是否描述为9，一个很自然的方式来设计网络就是将图片编码成强壮的像素放到输入神经元中, 如果图片是一个64乘64的灰度图片，那么我们就需要$$ 4096 = 64 x 64 $$的输入神经元，他们各自的强度在0到1之间适当的调整，输出层只需要包含一个神经元，输出值小于0.5表明他不是9，大于0.5表明它是9。
